...
def sort_vertices_and_faces_and_labels_and_features(vertices_, faces_,labels_,features_, num_tokens=256):
    vertices = np.clip((vertices_ + 0.5), 0, 1) * num_tokens  # type: ignore
    vertices_quantized_ = vertices.round().astype(int)
    ...
    sorted_indices = sorted(enumerate(faces), key=lambda x: tuple(sorted(x[1])))
    sorted_indices = [index for index,face in sorted_indices]
    face_ids = [face_ids[idx] for idx in sorted_indices]
    faces.sort(key=lambda f: tuple(sorted(f)))
    ...
    # 上述是原注释，但是我疑惑，明明应该是将y,x,z转换为 z,x,y才对,如果要按照注释的话，则输入的点的坐标应该是y,z,x,但过于违背常识，倾向于认为注释错了
    vertices = np.stack([vertices[:, 1], vertices[:, 0], vertices[:, 2]], axis=-1)
    labels = labels_[face_ids]
    features = features_[face_ids]
    return vertices, faces, labels,features
