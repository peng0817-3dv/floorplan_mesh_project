...
class WithoutPE(pl.LightningModule):
    def __init__(self, config):
        ...
        self.encoder = GraphEncoderWithoutPE(no_max_pool=config.g_no_max_pool, aggr=config.g_aggr, graph_conv=config.graph_conv, use_point_features=config.use_point_feats, output_dim=576,embed_dim=config.torch_embedding_dim)
...
class GraphEncoderWithoutPE(nn.Module):

    def __init__(self, no_max_pool=True, aggr='mean', graph_conv="edge", use_point_features=False, output_dim=512, embed_dim=8):
        super().__init__()
        ...
        self.gc1 = get_conv(self.conv, self.embed_dim * 9 + 7, 64, aggr=aggr)
    ...
    def embedder(self, points, num_discrete_coors = 128, dim_coor_embed = 8):
        # points: shape (N, 9)

        embed_func = nn.Embedding(num_discrete_coors, dim_coor_embed,device=points.device)
        discrete_points = self.discretize(points, num_discrete=num_discrete_coors)
        embed_points = embed_func(discrete_points)
        return embed_points.view(embed_points.shape[0], -1)
    ...
    def forward(self, x, edge_index, batch):
        ...
        x_em = self.embedder(x[:, :9],dim_coor_embed=self.embed_dim) # 位置单个维度的嵌入特征 8维，对比之前的63维
        x_n = x[:, 9:12]  # 法向量特征
        x_ar = x[:, 12:13]  # 面积特征
        x_an_0 = x[:, 13:14]  # 内角1
        x_an_1 = x[:, 14:15]  # 内角2
        ...
        x = torch.cat([x_em, x_n, x_ar, x_an_0, x_an_1, x_an_2], dim=-1)
        #
...
@hydra.main(config_path='../config', config_name='pe', version_base='1.2')
def main(config):
    # 创建一个lightning提供的Trainer实例
    trainer = create_trainer("TriangleTokens", config)
    model = WithoutPE(config)
