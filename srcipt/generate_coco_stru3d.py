...
def main():
    ouf_folder = r'G:\workspace_plane2DDL\augment_point_cloud_density'
    ...
    output_annotation_folder = os.path.join(ouf_folder, 'annotations')
    ### prepare

    if not os.path.exists(ouf_folder):
        os.mkdir(ouf_folder)
    if not os.path.exists(output_annotation_folder):
        os.mkdir(output_annotation_folder)

    train_img_folder = os.path.join(ouf_folder, 'train')
    val_img_folder = os.path.join(ouf_folder, 'val')
    test_img_folder = os.path.join(ouf_folder, 'test')
...
def visualize_result():
    ...
    visualization_seg(3251, annotation_json_path, img_folder)#2104
