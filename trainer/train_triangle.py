...
class GraphTransformerEncoder(pl.LightningModule):
    def __init__(self, config):
        ...
        self.train_dataset = FPTriangleWithGeneratedFeaturesAndLabel3ClsNodes(config, 'train', "scene_names")
        self.val_dataset = FPTriangleWithGeneratedFeaturesAndLabel3ClsNodes(config, 'val', "scene_names")

        # 网络的编码器为GraphEncoder，解码器为resnet34_decoder
        self.encoder = GraphEncoderAdaptExtraFeatures(no_max_pool=config.g_no_max_pool, aggr=config.g_aggr, graph_conv=config.graph_conv,
                                    use_point_features=config.use_point_feats, output_dim=576)
        self.transformer = TransformerNet(config, dim=512)
        self.pre_quant = torch.nn.Linear(config.embed_dim * 3, 512)
        self.FFN = torch.nn.Linear(512, config.num_cls)
        self.register_buffer('smoothing_weight', torch.tensor([2, 10, 200, 10, 2], dtype=torch.float32)
    ...
    def training_step(self, data, batch_idx):
        ...
        # 将geometric_torch式的batch转化为普通torch式的batch
        ...
        decoded_x_conv = self.FFN(encoded_x_conv)
        ...
        self.log("train/loss", loss.item(), on_step=False, on_epoch=True, prog_bar=True, logger=True, sync_dist=True,batch_size=self.config.batch_size)
        ...
        self.log("train/classify_loss", classify_loss.item(), on_step=False, on_epoch=True, prog_bar=True, logger=True, sync_dist=True,batch_size=self.config.batch_size)
        self.log("train/angle_loss", angle_loss, on_step=False, on_epoch=True, prog_bar=True, logger=True, sync_dist=True,batch_size=self.config.batch_size)
    ...
    def validation_step(self, data, batch_idx):
        ...
        # 计算损失
        # 三角形分类损失（classify_loss）
        classify_loss = torch.nn.functional.cross_entropy(decoded_x.reshape(-1, decoded_x.shape[-1]),
                                                          data.y.reshape(-1), reduction='mean')
        # 角度正则损失 (angle_loss)
        angle_loss = self.angle_loss(data, decoded_x)
        # 总损失
        loss = self.config.angle_loss_weight * angle_loss + self.config.classify_loss_weight * classify_loss
        ...
        if not torch.isnan(loss).any():
            self.log(f"val/loss", loss.item(), add_dataloader_idx=False, on_step=False, on_epoch=True, prog_bar=False, logger=True, sync_dist=True,batch_size=self.config.batch_size)
        ...
        self.log("val/classify_loss", classify_loss.item(), on_step=False, on_epoch=True, prog_bar=False, logger=True, sync_dist=True,batch_size=self.config.batch_size)
        self.log("val/angle_loss", angle_loss, on_step=False, on_epoch=True, prog_bar=False, logger=True, sync_dist=True,batch_size=self.config.batch_size)
...
def main(config):
    ...
    model = GraphTransformerEncoder(config)
