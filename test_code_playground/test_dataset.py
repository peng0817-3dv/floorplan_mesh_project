...
from util.visualization import export_mesh_to_shp, plot_vertices_and_faces_with_labels, export_mesh_to_obj,plot_ground_truth_and_prediction
import hydra
...
def main(config):
    config.dataset_root = r"G:/workspace_plane2DDL/real_point_cloud_dataset\stru3d_featured_shp" # 测试数据集位置
    ...
    _, targets, vertices, faces, _, op = dataset.get_all_features_for_shape(0)
    plot_ground_truth_and_prediction(vertices=vertices, faces=faces, gt_labels=targets, pred_labels=targets,output_path=r"test.png")
